...
This is a project I completed from Datacamp.com
...
In this notebook, we'll be using data collected from a year-long survey of 1.2 million people with only a bachelor's degree by PayScale Inc., made available [here](http://online.wsj.com/public/resources/documents/info-Degrees_that_Pay_you_Back-sort.html?mod=article_inline) by the Wall Street Journal for their article [Ivy League's Big Edge: Starting Pay](https://www.wsj.com/articles/SB121746658635199271)
...
To begin, let's prepare by loading the following packages: `tidyverse`, `dplyr`, `readr`,`ggplot2`, `cluster`,and `factoextra`. We'll then import the data from **degrees-that-pay-back.csv**, and take a quick look at what we're working with.
...
degrees_clean <- degrees %>% 
    mutate_at(vars( Starting.Median.Salary:Percentile.90), function(x) as.numeric(gsub("[\\$,]","",x))) %>% mutate(Career.Percent.Growth = Career.Percent.Growth / 100)
```
...
k_means_data <- degrees_clean %>% select(Starting.Median.Salary, Mid.Career.Median.Salary, Percentile.10,Percentile.90) %>% scale()
...
it seems that our two methods so far disagree on the optimal number of clusters... Time to pull out the tie breaker.
